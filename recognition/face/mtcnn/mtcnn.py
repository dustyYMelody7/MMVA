...
# using for runing arcface_test.py
...
class MTCNN(nn.Module):
    ...
    Keyword Arguments:
        ...
        device {torch.device} -- The device on which to run neural net passes. Image tensors and
            model are copied to this device before running forward passes. (default: {None})
    """

    def __init__(
            self, image_size=(112, 112), margin=0, min_face_size=20,
            thresholds=[0.6, 0.7, 0.7], factor=0.709, post_process=False,
            select_largest=True, selection_method=None, device=None
    ):
        super().__init__()
    ...
    def detect(self, img, return_face=True):
        ...
        # print("points:", points[0].shape if points[0] is not None else 0)
        ...
        if return_face:
            faces = self.extract(img=img, batch_boxes=boxes, landmarks=points)
    ...
    def extract(self, img, batch_boxes, landmarks):
        ...
        faces = None
        for im, box_im, landmark in zip(img, batch_boxes, landmarks):
            if box_im is None:
                continue

            for i, box in enumerate(box_im):

                face = extract_face(im, landmark[i], self.image_size)
                if self.post_process:
                    face = fixed_image_standardization(face)
                if faces is None:
                    faces = face.unsqueeze(0)
                else:
                    faces = torch.cat((faces, face.unsqueeze(0)), dim=0)

        if faces is None:
            return None
        return faces.to(self.device)
